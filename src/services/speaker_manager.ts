...
import { MALE_VOICES, FEMALE_VOICES, ALL_VOICES, NARRATION_ROLE_NAME, NARRATION_VOICE, MALE_INDICATORS, FEMALE_INDICATORS, DEFAULT_NARRATION_PROMPT, MAX_VOICE_PROMPT_LENGTH } from '../constants/index.js';
...
export class SpeakerManager {
  ...
  private guessGender(name: string): 'male' | 'female' | null {
    ...
    for (const ch of name) {
      if (FEMALE_INDICATORS.includes(ch)) return 'female';
      if (MALE_INDICATORS.includes(ch)) return 'male';
    }
    return null;
  }
